...
function App() {
 ...
 const [casesType , setCasesType] = useState("cases")
  ...
  const onCountryChange = async (e) => {
    const stickCountry = e.target.value;

      const url = stickCountry === "worldwide" ? "https://disease.sh/v3/covid-19/all" : `https://disease.sh/v3/covid-19/countries/${stickCountry}`
       
      await fetch(url)
      .then((response) => response.json())
        .then((data) => {
         setCountry(stickCountry);
          setCountryInfo(data);
    ...
    <div className="app">
      ...
      <Card className="app__right">
        <CardContent>
          ...
          <h3 className="chart-heading">World Wide New {casesType}</h3>
